...
const SavingsGoalItemCard: React.FC<SavingsGoalItemCardProps> = ({ goal, onSelect }) => {
    ...
    return (
        ...
        >
            ...
            {/* Progress Section */}
            <div className={`w-full bg-[var(--bg-interactive)] rounded-full h-3 mb-2 overflow-hidden ${isAchieved ? 'border border-green-500/20' : ''}`}>
                <div 
            ...
            <div className="flex justify-between items-center text-sm text-[var(--text-secondary)] mb-4">
                <span className={`font-semibold`} style={{color: isAchieved ? 'var(--color-income)' : 'var(--color-savings)'}}>{progress.toFixed(1)}% Tercapai</span>
                <div>
