...
interface AddDebtFormProps {
    ...
    onSave: (debt: Omit<DebtItem, 'id' | 'payments'>) => void;
}
...
const AddDebtForm: React.FC<AddDebtFormProps> = ({ setView, onSave }) => {
    ...
    const [formData, setFormData] = useState({
        name: '',
        source: '', // This will hold the selected lender or 'Lainnya'
        totalAmount: '',
        monthlyInstallment: '',
        tenor: '',
        dueDate: '',
    ...
    const handleInputChange = (field: keyof Omit<typeof formData, 'source'>, value: string) => {
         const isNumeric = ['totalAmount', 'monthlyInstallment', 'tenor', 'dueDate'].includes(field);
    ...
    const isStepValid = useMemo(() => {
        ...
        if (step === 2) return formData.totalAmount.trim() !== '' && formData.monthlyInstallment.trim() !== '';
        if (step === 3) return formData.tenor.trim() !== '' && formData.dueDate.trim() !== '' && parseInt(formData.dueDate) >= 1 && parseInt(formData.dueDate) <= 31;
        return false;
        ...
        switch(step) {
                     ...
                     <div className="space-y-6">
                         <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" inputMode="numeric" placeholder="15.000.000" value={formData.totalAmount ? parseInt(formData.totalAmount).toLocaleString('id-ID') : ''} onChange={e => handleInputChange('totalAmount', e.target.value)} className="w-full p-3 pl-9 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-[var(--primary-500)] focus:border-transparent text-right" />
                            </div>
                         ...
                         <div>
                            ...
                            <div className="relative">
                                <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500">Rp</span>
                                <input type="text" inputMode="numeric" placeholder="800.000" value={formData.monthlyInstallment ? parseInt(formData.monthlyInstallment).toLocaleString('id-ID') : ''} onChange={e => handleInputChange('monthlyInstallment', e.target.value)} className="w-full p-3 pl-9 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-[var(--primary-500)] focus:border-transparent text-right" />
                            </div>
            ...
            case 3:
                 return (
                     <div className="space-y-6">
                         <div>
                             <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tenor</label>
                             <div className="relative">
                                <input type="text" inputMode="numeric" placeholder="36" value={formData.tenor} onChange={e => handleInputChange('tenor', e.target.value)} className="w-full p-3 pr-14 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-[var(--primary-500)] focus:border-transparent text-right" />
                                 <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500">bulan</span>
                             </div>
                              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Jangka waktu pinjaman dalam bulan.</p>
                         </div>
                          <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" inputMode="numeric" min="1" max="31" placeholder="10" value={formData.dueDate} onChange={e => handleInputChange('dueDate', e.target.value)} className="w-full p-3 pl-20 bg-gray-50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-[var(--primary-500)] focus:border-transparent text-center" />
                            </div>
